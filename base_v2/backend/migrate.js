...
async function runMigrations() {
  try {
    // Initialize the database with the base schema first
    const baseSchemaPath = path.join(__dirname, '..', 'db', 'base_v2.sql');
    const baseSchemaSQL = fs.readFileSync(baseSchemaPath, 'utf8');
    const schemaStatements = baseSchemaSQL.split(';').filter(stmt => stmt.trim());
