...
router.get('/', [
  ...
  try {
    ...
    const featureToggles = await dbMethods.all(db, 
      'SELECT * FROM base_feature_toggle ORDER BY feature_name', 
      []
    );
    ...
    eventBus.emit('log:activity', {
      ...
      user_action: 'FEATURE_TOGGLE_LIST_VIEW',  // Fixed field name from action to user_action
      activity_details: 'Viewed all feature toggles',  // Fixed field name from details to activity_details
      user_ip_address: req.ip,  // Fixed field name from ip_address to user_ip_address
...
router.get('/:name', [
  ...
  try {
    ...
    const featureToggle = await dbMethods.get(db, 
      'SELECT * FROM base_feature_toggle WHERE feature_name = ?', 
      [toggleName]
    );
...
router.patch('/update', [
  ...
  try {
    ...
    await dbMethods.run(db, 
      'UPDATE base_feature_toggle SET is_enabled = ? WHERE feature_name = ?', 
      [isEnabledValue, name]
    );
