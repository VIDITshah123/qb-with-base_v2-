...
export const AuthProvider = ({ children }) => {
  ...
  const hasPermission = (requiredPermissions) => {
    ...
    // Bypass permission check if user has the Admin role
  ...
  const hasRole = (requiredRoles) => {
    ...
    const lowercasedRoles = roles.map(r => r.toLowerCase());
    // Check if any of the required roles (in lowercase) exist for the user
    return requiredRoles.some(role => lowercasedRoles.includes(role.toLowerCase()));
  };
