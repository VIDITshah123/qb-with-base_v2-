...
const RoleList = () => {
  ...
  useEffect(() => {
    if (roles.length > 0) {
      ...
      if (filters.name) {
        filtered = filtered.filter(role =>
          role.role_name.toLowerCase().includes(filters.name.toLowerCase())
        );
      }
      
      if (filters.description) {
        filtered = filtered.filter(role =>
          role.role_description.toLowerCase().includes(filters.description.toLowerCase())
        );
      }
  ...
  const groupedPermissions = permissionsMemo.reduce((acc, permission) => {
    // Check if permission name exists before trying to split it
    const category = permission && permission.name ? permission.name.split('_')[0] : 'other';
    if (!acc[category]) {
      acc[category] = [];
    }
  ...
  const handleSelectRole = (roleId) => {
    ...
    if (role && ['Admin', 'System'].includes(role.role_name)) {
      toast.warning(`The ${role.role_name} role is a system role and cannot be selected for bulk operations.`);
      return;
    }
  ...
  const handleBulkDelete = async () => {
    ...
    const systemRoleIds = roles
      .filter(role => ['Admin', 'System'].includes(role.role_name))
                      ...
                      <tr key={role.role_id}>
                        <td>
                          <Form.Check
                            ...
                            aria-label={`Select role ${role.role_name}`}
                            // Disable checkbox for system roles
                            disabled={['Admin', 'System'].includes(role.role_name)}
                          />
                        </td>
                        ...
                        <td>
                          <div className="d-flex align-items-center">
                            ...
                            {canDeleteRole && !['Admin', 'System'].includes(role.role_name) ? (
                              <Button 
                            ...
                            {!(canEditRole || (canDeleteRole && !['Admin', 'System'].includes(role.role_name))) && (
      ...
      <Modal show={showBulkDeleteModal} onHide={() => setShowBulkDeleteModal(false)} centered>
        ...
        <Modal.Body>
          ...
          {selectedRoles.some(roleId => {
            const role = roles.find(r => r.role_id === roleId);
            return role && ['Admin', 'System'].includes(role.role_name);
          }) && (
