...
const PermissionList = () => {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'permission_name', direction: 'asc' });
  ...
  useEffect(() => {
    if (permissions.length > 0) {
      ...
      if (selectedCategory !== 'all') {
        filtered = filtered.filter(permission => 
          permission.permission_name.startsWith(selectedCategory + '_')
        );
      }
  ...
  useEffect(() => {
    if (permissions.length > 0) {
      const uniqueCategories = [...new Set(
        permissions.map(permission => permission.permission_name ? permission.permission_name.split('_')[0] : 'other').filter((value, index, self) => self.indexOf(value) === index)
      )];
              ...
              <Table hover bordered className="align-middle">
                <thead>
                  <tr>
                    ...
                    >
                      Permission Name {getSortIcon('permission_name')}
                    </th>
                ...
                <tbody>
                  {filteredPermissions.length > 0 ? (
                    filteredPermissions.map((permission, index) => {
                      const category = permission?.permission_name ? permission.permission_name.split('_')[0] : 'other';
                      const action = permission?.permission_name ? permission.permission_name.substring(category.length + 1) : '';
...
const MissingRoutesContent = ({ missingRoutes, onCreateRoutes, creatingRoutes }) => {
  ...
  const handleSelectAll = (category) => {
    ...
    const isAllSelected = categoryPermissions.every(p => selectedPermissions.some(sp => sp.permission_name === p.permission_name));
    
    if (isAllSelected) {
      // Deselect all from this category
      setSelectedPermissions(prev => prev.filter(p => !categoryPermissions.some(cp => cp.permission_name === p.permission_name)));
    } else {
      // Select all from this category
      const newSelections = categoryPermissions.filter(p => !selectedPermissions.some(sp => sp.permission_name === p.permission_name));
  ...
  const handleTogglePermission = (permission) => {
    setSelectedPermissions(prev => {
      const exists = prev.some(p => p.permission_name === permission.permission_name);
      if (exists) {
        return prev.filter(p => p.permission_name !== permission.permission_name);
      } else {
    ...
    <div>
      ...
      <Accordion>
        {Object.entries(missingRoutes.missing_by_category || {}).map(([category, permissions]) => {
          const selectedInCategory = permissions.filter(p => selectedPermissions.some(sp => sp.permission_name === p.permission_name)).length;
              ...
              <Accordion.Body>
                ...
                <ListGroup variant="flush">
                  {permissions.map((permission, index) => {
                    const isSelected = selectedPermissions.some(p => p.permission_name === permission.permission_name);
                    return (
                      <ListGroup.Item 
                        key={index}
                          ...
                          <Form.Check
                            ...
                            label={
                              <div>
                                ...
                                <br />
                                <small className="text-muted">{permission.permission_description}</small>
                              </div>
                            }
                          />
                        </div>
              ...
              {rolesWithPermissions.map(role => (
                <div key={role.role_id} className="mb-3">
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    <h6 className="mb-0">{role.role_name || 'Unnamed Role'}</h6>
                    <Badge bg="primary">{role.permission_count || 0} permissions</Badge>
                  </div>
                  <div className="ps-3">
                    {role.permissions && role.permissions.length > 0 ? (
                      <div className="d-flex flex-wrap gap-1">
                        {role.permissions && role.permissions.slice(0, 5).map(permission => (
                          <Badge key={permission.permission_id} bg="secondary" className="small">
                            {permission.permission_name && permission.permission_name.length > 20 ? permission.permission_name.substring(0, 20) + '...' : (permission.permission_name || 'Unnamed')}
                          </Badge>
                        ))}
                        {role.permissions && role.permissions.length > 5 && (
                          <Badge bg="light" text="dark" className="small">
                            +{role.permissions ? role.permissions.length - 5 : 0} more
                          </Badge>
                        )}
                      </div>
                    ) : (
