...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaFilter, FaToggleOn, FaToggleOff, FaUserTag, FaUsersCog, FaSort, FaSortUp, FaSortDown, FaUser } from 'react-icons/fa';
...
const UserList = () => {
  ...
  const applyFilters = (usersToFilter = allUsers) => {
    ...
    if (filters.email) {
      filteredResults = filteredResults.filter(user => 
        user.user_email.toLowerCase().includes(filters.email.toLowerCase())
      );
    }
    ...
    if (filters.role) {
      filteredResults = filteredResults.filter(user => 
        user.roles && user.roles.some(role => 
          role.role_name.toLowerCase().includes(filters.role.toLowerCase())
        )
      );
    }
      ...
      filteredResults.sort((a, b) => {
        ...
        switch (sortConfig.key) {
          ...
          case 'role':
            ...
            aValue = a.roles && a.roles.length > 0 ? a.roles[0].role_name : '';
            bValue = b.roles && b.roles.length > 0 ? b.roles[0].role_name : '';
            break;
          case 'status':
            aValue = a.is_active;
            bValue = b.is_active;
                      ...
                      {canEditUser ? (
                        <Dropdown>
                          <Dropdown.Toggle variant="outline-secondary" size="sm" id={`role-dropdown-${user.user_id}`}>
                            {user.roles && user.roles.length > 0 ? user.roles.map(role => role.role_name).join(', ') : 'No Role'}
                          </Dropdown.Toggle>
                          <Dropdown.Menu>
                            {roles.map(role => (
                              <Dropdown.Item 
                    ...
                    <td>
                      {canEditUser ? (
                        <Button 
                          ...
                          disabled={user.user_email === 'admin@example.com'} // Protect admin account
                        >
                          {user.is_active ? 
