...
const checkPaymentFeatureEnabled = async (req, res, next) => {
  try {
    ...
    const featureToggle = await dbMethods.get(db, 
      'SELECT is_enabled FROM base_feature_toggle WHERE feature_name = ?', 
      ['payment_integration']
    );
...
const initializeDatabase = async (db) => {
  try {
    // Create base_payment_qr table if it doesn't exist
    ...
    await dbMethods.run(db, `
      CREATE TABLE IF NOT EXISTS base_payment_transactions (
        ...
        payment_descriptionid INTEGER,
        user_id INTEGER,
        transaction_notes TEXT,
        transaction_metadata TEXT,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (payment_descriptionid) REFERENCES base_payment_qr(id),
...
router.post('/qr-codes', [
  ...
  checkPaymentFeatureEnabled,
  upload.single('payment_qr_image_location'),
...
router.put('/qr-codes/:id', [
  ...
  try {
    ...
    const existingQrCode = await dbMethods.get(db, 'SELECT id FROM base_payment_qr WHERE id = ?', [qrCodeId]);
    if (!existingQrCode) {
      return res.status(404).json({ error: 'QR code not found' });
    }
    ...
    // Update QR code
    await dbMethods.run(db, 
      `UPDATE base_payment_qr SET ${updateFields.join(', ')} WHERE id = ?`, 
      updateValues
    );
...
router.delete('/qr-codes/:id', [
  ...
  try {
    ...
    const qrCode = await dbMethods.get(db, 
      'SELECT id, image_url FROM base_payment_qr WHERE id = ?', 
      [qrCodeId]
    );
    ...
    const transactionsUsingQrCode = await dbMethods.get(db, 
      'SELECT COUNT(*) as count FROM base_payment_transactions WHERE payment_descriptionid = ?', 
      [qrCodeId]
    );
    ...
    // Delete QR code
    await dbMethods.run(db, 'DELETE FROM base_payment_qr WHERE id = ?', [qrCodeId]);
    
    // Delete file from filesystem if it exists
    if (qrCode.image_url && fs.existsSync(qrCode.image_url)) {
      fs.unlinkSync(qrCode.image_url);
    }
...
router.patch('/qr-codes/:id/activate', [
  ...
  try {
    ...
    try {
      ...
      await dbMethods.run(db, 'UPDATE base_payment_qr SET active = 0');
      
      // Activate the selected QR code
      await dbMethods.run(db, 'UPDATE base_payment_qr SET active = 1 WHERE id = ?', [qrCodeId]);
...
router.patch('/qr-codes/:id/deactivate', [
  ...
  try {
    ...
    // Check if QR code exists
    const existingQrCode = await dbMethods.get(db, 'SELECT id, active FROM base_payment_qr WHERE id = ?', [qrCodeId]);
    if (!existingQrCode) {
      return res.status(404).json({ error: 'QR code not found' });
    }
    
    // Check if QR code is already inactive
    if (existingQrCode.active === 0) {
      return res.status(400).json({ error: 'QR code is already inactive' });
    }
    
    // Deactivate the QR code
    await dbMethods.run(db, 'UPDATE base_payment_qr SET active = 0 WHERE id = ?', [qrCodeId]);
  ...
  try {
    ...
    let sql = `
      SELECT 
        ...
        t.currency,
        t.payment_status,
        t.payment_descriptionid,
        t.user_id,
        t.transaction_date,
        t.transaction_notes,
        t.transaction_metadata,
        q.name as payment_qr_name,
        u.first_name || ' ' || u.last_name as user_name
      FROM base_payment_transactions t
      LEFT JOIN base_payment_qr q ON t.payment_descriptionid = q.id
    ...
    if (search) {
      whereConditions.push('(t.transaction_ref LIKE ? OR q.name LIKE ?)');
...
router.get('/qr-codes/:id/image', [
  ...
  try {
    ...
    const qrCode = await dbMethods.get(db, 
      'SELECT image_url FROM base_payment_qr WHERE id = ?', 
      [qrCodeId]
    );
    ...
    // If image_url exists in database (stored as BLOB)
    if (qrCode.image_url) {
      res.set('Content-Type', 'image/png');
      return res.send(Buffer.from(qrCode.image_url));
    } 
    // If we have a file path, try to serve that
    else if (qrCode.image_url && fs.existsSync(qrCode.image_url)) {
      return res.sendFile(qrCode.image_url);
    } 
    // No image found
    else {
...
router.post('/transactions', [
  ...
  try {
    ...
    if (payment_descriptionid) {
      const qrCode = await dbMethods.get(db, 'SELECT id FROM base_payment_qr WHERE id = ?', [payment_descriptionid]);
      if (!qrCode) {
        return res.status(404).json({ error: 'QR code not found' });
      }
    }
    ...
    const existingTransaction = await dbMethods.get(db, 
      'SELECT payment_descriptionid FROM base_payment_transactions WHERE transaction_ref = ?', 
      [transaction_ref]
    );
    ...
    eventBus.emit('log:activity', {
      ...
      details: `Created new transaction: ${transaction_ref}`,
      ip_address: req.ip,
      user_agent: req.headers['user-agent']
    });
